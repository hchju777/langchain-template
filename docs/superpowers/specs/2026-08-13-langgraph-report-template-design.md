...
## 10-1. 구현 후 갱신 (2026-08-13)

이 스펙을 구현하면서 드러난 차이를 기록한다.

**슬롯 override에 서브그래프를 쓸 수 있다.** §3의 "상속은 코드에서, 선택은 config에서"를 처음 구현했을 때 동작하지 않았다. 레지스트리가 서브그래프용과 노드 부품용으로 나뉘어 있는데 `nodes` override가 후자만 봤기 때문이다. 지금은 둘 다 받고, 서브그래프를 지정하면 **그 클래스로 인스턴스를 따로 만들어** 슬롯을 가져온다(메서드만 빌려오면 그 클래스의 헬퍼에 접근할 수 없다).

공장별 특수화 클래스는 다른 공장 config에 등장하지 않아 고아 검증에 걸린다. 그래서 `@register(slot_only=True)`로 "독립 실행용이 아님"을 선언한다.

**`ports` 바인딩(§5)은 미구현이다.** config로 저장소를 고르는 기능이 없고, `build_dependencies`가 어댑터를 하드코딩하며 `Dependencies.redis`처럼 필드명이 기술 이름이다. 서브그래프가 저장소 종류를 알게 되므로 §5의 의도와 어긋난다.

**§10의 테스트는 하나도 작성되지 않았다.** 어댑터 테스트만 의도적으로 뺀 것이 아니라 노드·서브그래프·config·전체그래프 4개 층 전부 미작성이다.

**LLM은 서브그래프별로 교체할 수 있다.** §6에는 없던 것으로, `subgraphs.<name>.llm`이 기본 설정 위에 deep merge된다.
