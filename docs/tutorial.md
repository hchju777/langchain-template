...
> 시작 전에 [README](../README.md)를 훑어두면 이해가 빠릅니다. 실행 중에 무엇이 어떻게 도는지는 [아키텍처](architecture.md)에 있습니다. 안 봐도 따라올 수는 있습니다.
...
어떤 모델이 실제로 쓰였는지는 `LLMTrace`에 남아 State에 쌓입니다. 자세한 건 [README의 LLM 연결과 모델 교체](llm.md#실제-llm으로-전환)를 보세요.
